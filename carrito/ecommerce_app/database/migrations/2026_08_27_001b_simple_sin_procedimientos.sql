-- =============================================================================
--  VERSIÓN ALTERNATIVA de la migración — sin procedimientos almacenados
--
--  Usar SOLO si la migración principal
--  (2026_08_27_001_ecommerce_b2b.sql) falla con un error del tipo
--  "Access denied ... CREATE ROUTINE" o si phpMyAdmin no procesa el DELIMITER.
--
--  Diferencia: acá los ALTER van directos, así que NO es reejecutable.
--  Si al correrla ves alguno de estos errores, IGNORALO y seguí con el
--  siguiente bloque: significa que esa parte ya estaba hecha.
--
--      #1060 - Duplicate column name    -> la columna ya existe
--      #1061 - Duplicate key name       -> el índice ya existe
--      #1050 - Table already exists     -> la tabla ya existe
--
--  En phpMyAdmin conviene ejecutar bloque por bloque (pestaña SQL),
--  no importar el archivo entero, para poder saltear esos errores.
-- =============================================================================

SET NAMES utf8mb4;

-- ── BLOQUE 1 ─ Columna de códigos de función ─────────────────────────────────
...
-- ── BLOQUE 2 ─ Tablas del storefront (seguras de reejecutar) ─────────────────
...
-- ── BLOQUE 3 ─ Módulo, páginas, funciones y perfil ───────────────────────────
--  Este bloque SÍ es reejecutable: todo va con NOT EXISTS.
--  Es el que resuelve el error "Falta instalar el módulo del ecommerce".
...
-- ── BLOQUE 4 ─ Índices de apoyo (opcionales) ─────────────────────────────────
--  Mejoran la performance del catálogo y de los permisos.
--  Si alguno da error #1061, ya existía: seguí con el siguiente.
...
-- ── CONTROL ──────────────────────────────────────────────────────────────────
--  Tiene que devolver funciones_ecom = 4 y perfiles_cliente_web >= 1
