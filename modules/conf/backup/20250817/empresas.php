...
<script>
// Add this function to load localidades
function cargarLocalidades() {
    $.get('empresas_ajax.php', {accion: 'obtener_localidades'}, function(res) {
        var select = $('#localidad_id');
        select.empty();
        select.append('<option value="">Seleccionar Localidad</option>');
        $.each(res, function(index, localidad) {
            select.append(`<option value="${localidad.localidad_id}">${localidad.localidad}</option>`);
        });
    }, 'json');
}
