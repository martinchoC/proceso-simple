...
function obtenerTiposTabla($conexion) {
    $sql = "SELECT * FROM conf__tablas_tipos WHERE estado_registro_id = 1 ORDER BY tabla_tipo";
    $res = mysqli_query($conexion, $sql);
    $data = [];
    while ($fila = mysqli_fetch_assoc($res)) {
        $data[] = $fila;
...
function agregarTabla($conexion, $data) {
    ...
    $sql = "INSERT INTO conf__tablas 
            (tabla_nombre, tabla_descripcion, modulo_id, tabla_tipo_id, estado_registro_id) 
            VALUES 
            ('$tabla_nombre', '$tabla_descripcion', $modulo_id, $tabla_tipo_id, $estado_registro_id)";
...
function cambiarEstadoTabla($conexion, $id, $nuevo_estado) {
    ...
    $sql = "UPDATE conf__tablas SET estado_registro_id = $nuevo_estado WHERE tabla_id = $id";
    return mysqli_query($conexion, $sql);
}
