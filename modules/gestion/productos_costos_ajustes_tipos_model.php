...
/**
 * Modelo para gestión de Tipos de Ajuste de Costos
...
function obtenerBotonAgregar($conexion, $pagina_id)
{
    ...
    return [
        'nombre_funcion' => 'Agregar Tipo de Ajuste',
        'accion_js' => 'agregar',
...
function ejecutarTransicionEstado($conexion, $tipo_id, $accion_js, $empresa_idx, $pagina_id)
{
    $tipo_id = intval($tipo_id);
    $pagina_id = intval($pagina_id);

    $sql_check = "SELECT producto_costo_ajuste_tipo_id, tabla_estado_registro_id 
                  FROM gestion__productos_costos_ajustes_tipos 
                  WHERE producto_costo_ajuste_tipo_id = ?";
    ...
    $sql_update = "UPDATE gestion__productos_costos_ajustes_tipos 
    ...
    mysqli_stmt_bind_param($stmt, "ii", $estado_destino_id, $tipo_id);
...
function obtenerTiposAjuste($conexion, $empresa_idx, $pagina_id)
{
    $pagina_id = intval($pagina_id);
    ...
    $sql = "SELECT t.*, 
            ...
            FROM gestion__productos_costos_ajustes_tipos t
            LEFT JOIN conf__estados_registros er ON t.tabla_estado_registro_id = er.estado_registro_id
            LEFT JOIN conf__colores c ON er.color_id = c.color_id
            ORDER BY t.orden, t.producto_costo_ajuste_tipo_nombre";
...
// Agregar nuevo tipo de ajuste
function agregarTipoAjuste($conexion, $data)
{
    $codigo = mysqli_real_escape_string($conexion, trim($data['producto_costo_ajuste_tipo_codigo'] ?? ''));
    $nombre = mysqli_real_escape_string($conexion, trim($data['producto_costo_ajuste_tipo_nombre'] ?? ''));
    ...
    $sql_check = "SELECT producto_costo_ajuste_tipo_id FROM gestion__productos_costos_ajustes_tipos WHERE producto_costo_ajuste_tipo_codigo = ?";
    $stmt = mysqli_prepare($conexion, $sql_check);
    if ($stmt) {
    ...
    $sql = "INSERT INTO gestion__productos_costos_ajustes_tipos 
            (producto_costo_ajuste_tipo_codigo, producto_costo_ajuste_tipo_nombre, descripcion, orden, tabla_estado_registro_id) 
            VALUES (?, ?, ?, ?, ?)";
    ...
    mysqli_stmt_bind_param($stmt, "sssii", $codigo, $nombre, $descripcion, $orden, $estado_inicial);
    $success = mysqli_stmt_execute($stmt);

    if ($success) {
        $tipo_id = mysqli_insert_id($conexion);
        mysqli_stmt_close($stmt);
        return ['resultado' => true, 'producto_costo_ajuste_tipo_id' => $tipo_id];
    } else {
        mysqli_stmt_close($stmt);
        return ['resultado' => false, 'error' => 'Error al crear el tipo de ajuste: ' . mysqli_error($conexion)];
    }
}

// Editar tipo de ajuste existente
function editarTipoAjuste($conexion, $id, $data)
{
    $id = intval($id);
    $codigo = mysqli_real_escape_string($conexion, trim($data['producto_costo_ajuste_tipo_codigo'] ?? ''));
    $nombre = mysqli_real_escape_string($conexion, trim($data['producto_costo_ajuste_tipo_nombre'] ?? ''));
    ...
    $sql_check = "SELECT producto_costo_ajuste_tipo_id FROM gestion__productos_costos_ajustes_tipos 
                  WHERE producto_costo_ajuste_tipo_codigo = ? AND producto_costo_ajuste_tipo_id != ?";
    $stmt = mysqli_prepare($conexion, $sql_check);
    if ($stmt) {
    ...
    $sql = "UPDATE gestion__productos_costos_ajustes_tipos 
            SET producto_costo_ajuste_tipo_codigo = ?, 
                producto_costo_ajuste_tipo_nombre = ?, 
                descripcion = ?, 
                orden = ?
            WHERE producto_costo_ajuste_tipo_id = ?";
    ...
    mysqli_stmt_bind_param($stmt, "sssii", $codigo, $nombre, $descripcion, $orden, $id);
    $success = mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    ...
    if ($success) {
        ...
        return ['resultado' => false, 'error' => 'Error al actualizar el tipo de ajuste: ' . mysqli_error($conexion)];
    }
}

// Obtener tipo de ajuste específico
function obtenerTipoAjustePorId($conexion, $id)
{
    $id = intval($id);

    $sql = "SELECT * FROM gestion__productos_costos_ajustes_tipos WHERE producto_costo_ajuste_tipo_id = ?";
    $stmt = mysqli_prepare($conexion, $sql);
    if (!$stmt)
        return null;
