...
try {
    switch ($accion) {
        case 'listar':
            $bocas = obtenerBocas($conexion, $empresa_idx, $pagina_idx);
        ...
        case 'obtener_localidades_por_provincia':
            $provincia_id = intval($_GET['provincia_id'] ?? 0);
            if (empty($provincia_id)) {
                echo json_encode([], JSON_UNESCAPED_UNICODE);
                break;
            }
            $localidades = obtenerLocalidadesPorProvincia($conexion, $provincia_id);
        ...
        case 'ejecutar_accion':
            ...
            $resultado = ejecutarTransicionEstado($conexion, $boca_id, $accion_js, $empresa_idx, $pagina_id);
        ...
        case 'agregar':
            $data = [
                ...
                'es_deposito' => isset($_POST['es_deposito']) ? intval($_POST['es_deposito']) : 0,
                'boca_nombre' => trim($_POST['boca_nombre'] ?? ''),
            ...
            $resultado = agregarBoca($conexion, $data);
            echo json_encode($resultado, JSON_UNESCAPED_UNICODE);
            break;

        case 'editar':
            $id = intval($_POST['boca_id'] ?? 0);
            $data = [
                'sucursal_id' => intval($_POST['sucursal_id'] ?? 0),
                'es_deposito' => isset($_POST['es_deposito']) ? intval($_POST['es_deposito']) : 0,
                'boca_nombre' => trim($_POST['boca_nombre'] ?? ''),
            ...
            $resultado = editarBoca($conexion, $id, $data);
            echo json_encode($resultado, JSON_UNESCAPED_UNICODE);
            break;

        case 'obtener':
            $id = intval($_POST['boca_id'] ?? $_GET['boca_id'] ?? 0);
            if (empty($id)) {
                echo json_encode(['error' => 'ID no proporcionado'], JSON_UNESCAPED_UNICODE);
                break;
            }
            $boca = obtenerBocaPorId($conexion, $id, $empresa_idx);
            if ($boca) {
                echo json_encode($boca, JSON_UNESCAPED_UNICODE);
            } else {
                echo json_encode(['error' => 'Boca no encontrada'], JSON_UNESCAPED_UNICODE);
            }
            break;
    ...
    error_log("Excepción en bocas_ajax.php: " . $e->getMessage());
